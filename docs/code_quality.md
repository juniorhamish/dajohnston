...
This document outlines the strategy for measuring and maintaining high code quality across the Multi-App Portal project. We aim for automated enforcement to keep the effort low while ensuring the system remains scalable and maintainable.
...
*   **Next.js (Frontend):**
    *   **Biome:** An all-in-one tool for linting, formatting, and organizing imports. It replaces ESLint and Prettier with a single, high-performance binary.
...
*   **Target:** We aim for **80% line coverage** for all new code. Critical business logic (e.g., Auth, Tenantization) should aim for **95%+**.
...
For a high-level overview of technical debt, code smells, and security vulnerabilities, we integrate with **SonarCloud** (free for public projects on GitHub).
...
*   **Husky & lint-staged:** Automatically runs `biome check --apply` on changed files before every `git commit`.
...
*   **Data Privacy:** Double-checking that tenantization/RLS logic is correctly implemented for new features.
